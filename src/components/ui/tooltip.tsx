...
export function Tooltip({
  ...
  // axis lift toward the trigger for the enter/exit motion
  const liftX = side === "left" ? 7 : side === "right" ? -7 : 0;
  const liftY = side === "top" ? 7 : side === "bottom" ? -7 : 0;

  const containerClass = horizontal
    ? "pointer-events-none absolute top-1/2 z-50 flex h-0 items-center"
  ...
  return (
    <span className={`relative inline-flex ${className}`}>
      ...
      <span aria-hidden={!open} className={containerClass} style={containerStyle}>
        <AnimatePresence>
          {open && (
            <motion.span
              role="tooltip"
              ...
              initial={
                reduced
                  ...
                  : skipped
                    ...
                    : { opacity: 0, scale: 0.9, x: liftX, y: liftY, filter: "blur(4px)" }
              }
              ...
              className={`relative w-max max-w-[220px] shrink-0 overflow-hidden rounded-[8px] px-2 py-1 text-[11.5px] font-medium leading-snug text-popover-foreground ${contentClassName}`}
            >
              <motion.span
                aria-hidden
                layout={!reduced}
                transition={reduced ? { duration: 0 } : GLIDE}
                className="absolute inset-0 rounded-[8px] border border-border bg-popover shadow-md"
              />
              <motion.span
