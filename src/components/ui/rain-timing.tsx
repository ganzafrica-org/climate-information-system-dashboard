...
export const RainTimingDisplay: React.FC<RainTimingDisplayProps> = ({ hourly, className = '' }) => {
    ...
    return (
        <Card className={`border-0 shadow-md bg-gradient-to-br from-[#147677]/10 to-[#F5A623]/10 border-l-4 border-l-[#F5A623] ${className}`}>
            <CardHeader className="pb-3">
                <CardTitle className="text-base text-[#147677] flex items-center gap-2">
                    <CloudRain className="h-5 w-5 text-[#F5A623]" />
            ...
            <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                    <Badge variant="outline" className="bg-[#F5A623]/15 text-[#F5A623] border-[#F5A623]/40">
                ...
                {/* Peak Rain Time */}
                {rainTiming.peakRainTime && (
                    <div className="bg-white rounded-lg p-3 border border-[#147677]/20">
                        <div className="flex items-center gap-2 mb-2">
                            <TrendingUp className="h-4 w-4 text-[#147677]" />
                            <span className="font-medium text-[#147677]">{t('peakRainTime') || 'Peak Rain Time'}</span>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-sm">
                            <div>
                                <span className="text-gray-600">{t('time') || 'Time'}:</span>
                                <span className="font-semibold ml-1 text-[#147677]">{rainTiming.peakRainTime.time}</span>
                            </div>
                            <div>
                                <span className="text-gray-600">{t('amount') || 'Amount'}:</span>
                                <span className="font-semibold ml-1 text-[#147677]">{rainTiming.peakRainTime.amount}mm</span>
                            </div>
                            <div>
                                <span className="text-gray-600">{t('chance') || 'Chance'}:</span>
                                <span className="font-semibold ml-1 text-[#147677]">{rainTiming.peakRainTime.chance}%</span>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {rainTiming.rainStartsAt && rainTiming.rainEndsAt && (
                    <div className="bg-white rounded-lg p-3 border border-[#147677]/20">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="font-medium text-[#147677]">{t('rainWindow') || 'Rain Window'}</span>
                        </div>
                        <div className="text-sm text-[#147677]">
                ...
                <div className="space-y-3">
                    {hasPeriods(rainTiming.periods.morning) && (
                        <div>
                            <h4 className="text-sm font-medium text-[#147677] mb-2">
                                {getPeriodLabel('morning')} ({rainTiming.periods.morning.length} {t('periods') || 'periods'})
                            </h4>
                            <div className="space-y-2">
                                {rainTiming.periods.morning.map((period, idx) => (
                                    <div key={idx} className="bg-white rounded p-2 border border-[#147677]/10 text-sm">
                                        <div className="flex justify-between items-center">
                                            <span className="font-medium text-[#147677]">{period.time}</span>
                                            <div className="flex items-center gap-3">
                                                <span className="text-[#147677]">
