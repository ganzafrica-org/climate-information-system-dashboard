...
/**
 * Simple skeleton block (kept from the original shadcn primitive so existing
 * usages keep working). Use for one-off placeholder shapes.
 */
function Skeleton({
  className,
  ...props
...
/**
 * SkeletonSwap — interior.dev's ready→content crossfade. Reserve space for a
 * section, show an animated skeleton while `ready` is false, then crossfade to
 * the real content. Adapted to the app's `muted`/`foreground` tokens.
 */
...
function SkeletonSwap({
  ...
  return (
    <div
      ...
      style={{ height: box }}
      className={cn("relative grid overflow-y-auto overscroll-contain", className)}
    >
      <motion.div
        ref={body}
